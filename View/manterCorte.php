...
if(isset($_GET['id'])) {
    ...
    $resultado = $AgendaDAO->confirmarCorteDAO($Agenda);

    // Redireciona de volta para a página anterior após a exclusão
    header("Location: {$_SERVER['HTTP_REFERER']}?confirmacao=realizada");
    exit;
    ...
    header("Location: {$_SERVER['HTTP_REFERER']}?confirmacao=nao-realizada");
    exit;
}
?>